...
- Before a human gate, run `bun run agent:cli -- --base-url <actual-url> context --project-id <id> --script-id <id> ...`. Present only the returned `guiUrl`: it contains and validates the exact project, script, fixed revision, candidate, run, plan, and task identities that the CLI is using. A bare localhost URL or a guessed URL is not an acceptable handoff.
- The GUI is for the human gate and explicit UI validation. The production agent must not open or operate the handed-off GUI unless the user explicitly asks it to debug the UI. For external visual audit, download candidate images through the GURUGURU asset API with `agent:cli api ... --output` and inspect those files with the agent's vision capability.
