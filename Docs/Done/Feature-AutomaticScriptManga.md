...
- `src/shared/scriptMangaPlan.ts`: シーン境界を跨がず、action/synopsis段落を瞬間の境界として、同じ瞬間を既定で最大6要素/4発話まで1コマ、4コマまで1ページへまとめる決定的プランナー。台詞要素上限は1〜8で設定できる。1〜6コマに一致するlayoutを選び、各コマにsource element IDと`dialogue_lines.order_index`を保持する。台詞本文は画像promptへ転記せず、speech act・表情・mouth state等の視覚状態へ変換する。
...
- `src/server/scriptMangaDirector.ts`: `planningMode: "llm"` で OpenAI 互換LLMをネーム監督として挿入する。4ページ単位で画角・主役・単一アクション・感情・構図・英語画像プロンプトを構造化JSON化する。人物名・aliasは物語メタデータとしてのみ扱い、画像生成フィールドでは`primary character`などの中立な視覚役割名を使うよう監督へ指示する。ページ数・コマID・台詞対応はLLMに変更させず、既存の全発話保持保証を維持する。
...
- 複数人物のReferenceManifestは保持するが、現行生成requestの顔条件はfocal subject 1件である。regional conditioningと人物ごとの自動逐次inpaintは未実装。候補画像へ手動maskを描く局所repairは利用できる。
- taskの`retry`はコマ全体の再生成である。手動maskの局所inpaintとrepair lineageは実装済みだが、VLM violationからmaskを自動作成するrepair planner、lettering後のページ可読性監査、layout/PanelSpecを直接編集するvisual plan editorは未実装。
- アニメ顔推論のモデル実行は今回の検証後処理で行った。常設UIから完全自動実行するには、検出sidecarまたはWeb向け変換モデルの配布経路を追加する。

## 変更履歴

- 2026-07-19: ネーム監督へ、人物名・aliasを画像生成フィールドへ転記せず中立な視覚役割名で演出する指示を追加。一般名詞と一致する名前を機械的に除去せず、LLM指示だけで既存作品キャラクターへの連想を抑える方針とした。
- 2026-07-15: 1コマの台詞要素上限を1〜8・既定4としてAPI/UI/N1/決定的gateへ統一し、action境界、脚本順・scene・visible cast検証を強化。完全一致successor継承、手動mask局所repair、pose selector、JPEG、PPTX源暎アンチック指定を反映。
