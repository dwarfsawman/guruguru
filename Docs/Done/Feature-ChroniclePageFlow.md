...
- **Beat タグ**: 区間の先頭セリフを代表表示し、区間内のセリフ件数をバッジとホバー説明で示す。
- **Beat クリック**: タグ直下に縦並びの全セリフ一覧(話者・配置先ページ)を表示。現在ページの配置は強調し、
  対応する PageObject の保存済み本文を最優先し、無ければ `text_override` / `speaker_label_override` の
  固定本文・話者を表示する。
  ページ内に対応吹き出しがあれば選択できる。
...
- 2026-07-14: Beatタグが先頭セリフの代表表示であることを明示し、件数バッジ・ホバー説明を追加。
  クリック時の縦一覧を割り当て操作パネルより上へ移し、現在ページでは吹き出しの固定本文/話者を表示。
