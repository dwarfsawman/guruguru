...
- **キーボード**: `ArrowRight`・`Space`＝次、`ArrowLeft`＝前、`Escape`＝設定ドロワーを閉じる→無ければ Reader を
  閉じる。数値入力等 text-entry にフォーカス中はページ送りキーを奪わない。
...
- `npm run typecheck` / `npm test`（405 tests。`bookReader.test.ts` 14 件: normalize / single rtl・ltr /
  spread rtl start=1・2・3 / spread ltr start=2 / 最終ページ片側 / 画像なし混在 / 0ページ / 範囲外クランプ /
  canonical）/ `npm run build` すべてグリーン。
