...
`pagePanelLightboxController` の lightbox を「ページ編集画面」に発展させる。上部のモードタブは次の3つ:

- **レイヤ** — コマ画像と P1〜P3 のオブジェクトを最終合成と同じキャンバスで編集する。右サイドバーに
  レイヤ一覧、選択中のコマ/オブジェクト設定、Chronicle/セリフを表示する。
- **コマ枠** — P5。頂点編集・分割
- **モザイク** — P6。リージョン編集

旧「コマ」「オブジェクト」タブは「レイヤ」へ統合した。コマ画像レイヤを選択すると生成・クロップ操作を、
オブジェクトレイヤを選択すると変形・削除・プロパティ編集を行える。オブジェクトは前景/背景の同一帯域内で
ドラッグまたは上下ボタンにより並べ替え、保存済み `objects` の z 順へ反映する。コマ画像レイヤの順序は
レイアウトで固定する。「画像以外を隠す」と各レイヤの目アイコンは編集時だけの一時表示で、保存・書き出しには
影響しない。設定を開いても中央キャンバスの大きさが変わらないよう、設定とレイヤ一覧は独立スクロールする。

state は `pagePanelLightbox` に `mode` を持ち、旧 `panels` 値は `objects`（表示名「レイヤ」）へ正規化する。
レイアウトを持たないページ（1枚絵ページ）でもレイヤ/モザイクモードは開けるようにする
（CG集は1枚絵＋文字が主流のため。`page.layout` が無い場合は代表アセットのアスペクト比を page とみなす）。
...
- 対象: `polygon`（頂点ドラッグ・辺中点への頂点追加・頂点削除）、`rect` / `ellipse` の polygon 変換、
  構造化 `path.bezier`（アンカー・入出力制御点）。任意の閉じたマウス軌跡は等弧長で4〜18点へ整形し、
  Catmull-Rom 接線由来の閉じた cubic Bezier コマとして追加する。旧来の `path.d` のみの形状は表示のみ。
- polygon 頂点ドラッグは、隣接辺が水平・垂直または別のコマ辺へ近づくと完全平行へスナップし、
  基準辺・延長線・「水平/垂直/平行」ラベルをスマートガイド表示する。
- 外周辺の裁ち切りドラッグは、対象辺の破線だけでなくページ端の帯・トリム線・「裁ち切り」ラベルで
  仕上がりを予告し、pointerup で既存の bleed 座標へ確定する。
...
- 2026-07-18: P5へ平行スマートガイド、洗練した裁ち切りプレビュー、フリーハンド→編集可能cubic Bezierコマ枠を追加。
