...
## 実装後のフィードバック対応(2026-07-06、ブランチ `worktree-fix-paste-followups`)

- **グリッドプレビューへの合成表示 + PASTE タグ**: プロジェクト詳細 API が `pasteAttachments`(assetId → objects)を同梱し、グリッドタイルの `<img>` に `.paste-grid-canvas`(同じ intrinsic size + `object-fit: cover`)を重ねて貼り付けを合成表示。MASK/POSE と同様の `PASTE` バッジ(`state.showPasteGridTag`)で ON/OFF でき、OFF では合成も消える。
- **エッジポップアウトの操作性**: 本体クリックでは閉じない(展開トグルはフッタのみ)。クリックで focus が乗っている間は hover が外れても折りたたまず、ポップアウト外クリック(focus 喪失)で閉じる。添付サムネイルの展開は `grid-template-rows: 0fr⇄1fr` transition のアニメーションになった。
- **焼き込み(ペイントレイヤーへの bake)機能は削除**(ユーザー指示 2026-07-06)。パネルの操作行は 複製/前面へ/背面へ/削除 のみ。
- **WebSAM のエンコード入力**は「元画像+ペイントレイヤー+貼り付けオブジェクト」の見たまま合成に変更(貼り付けた被写体も SAM の選択対象になる)。
...
- 2026-07-06: フィードバック対応(冒頭の「実装後のフィードバック対応」参照)。グリッド合成表示+PASTE タグ、エッジポップアウトのクリック挙動/展開アニメーション、WebSAM 入力の合成化、**焼き込み機能の削除**。
