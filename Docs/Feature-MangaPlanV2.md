...
> 状態: 進行中。2026-07-15 時点で、immutableなrevision/plan、実行ライフサイクル、P2最小UI、LM Studio VLM監査、候補の人間採用、run直結export、successor runの完全一致asset継承、選択範囲の手動局所inpaint修復までを実装済み。VLM違反からの自動mask/repair計画は未実装。
...
Fountain revision
  ...
  → 必要なら既存候補を親に局所inpaintし、同じtaskへ候補追加
  → manualは人が候補をselect、VLMは合格候補だけを人がselect
```

VLMは人物同一性・行動整合・偽文字・連続性を補助評価するが、候補を自動採用しない。監査不能や不合格も候補を失わせず人間reviewへ戻す一方、VLM modeでは合格reportのある候補だけを採用できる。不合格時はコマ全体をretryするか、人がmaskを指定して同じtaskへ局所repair候補を追加する。VLM violationからmaskを自動作成するrepair plannerはまだない。
...
`validateMangaPlanV2`は、version、revision、ページ連番、layout snapshotのgeometryとコマ数、ID一意性、source/beat/world state/entity参照、bboxとsafe zone、全台詞snapshotの一度だけの割当、脚本順・scene一致・`dialogueOrderIndexes`、通常発話speakerとactionで肯定的に画面内へ置かれた人物のcast一致、`compiledPrompt`を決定的に検査する。`dialoguePolicy`は`preserve | adapt | fill`を一括経路で選べる。`preserve`は原文要素をそのまま配置し、`adapt`は原文一致の呼吸分割、`fill`は分割にcaption/monitor/SFXの派生unitを加える。自由な台詞生成を行う`generate`だけはlexical-similarity gate未実装のため400で拒否する。
...
run作成時の`targetPageCount`、`panelsPerPage`、`maxDialoguesPerPanel`（1〜8、既定4）はネーム密度を先に制約する。heuristicでも`targetPageCount`は1ページ1コマ〜`panelsPerPage`の範囲へ連続コマを均等配分するbest-effort目標として使い、空ページや上限違反は作らない。`maxDialoguesPerPanel`は台詞要素数のhard ceilingであり、吹き出し数だけによる品質規則ではない。近接台詞を同じコマへまとめられるのは、同じscene、瞬間、主行動を共有する場合だけで、最終可否は文字サイズと専有率のpreflightで判定する。`panelsPerPage`はページ内コマ数、`maxPanelCount`はplan全体のhard ceilingで、`maxPanelCount:0`は総コマ数上限なしを表す。3つの上限は画像生成task投入前に422で拒否し、provided/successorを含むcreate、plan PATCH、approve、start、resumeで再検査する。上位モデルによるN1候補生成は演出判断を改善するが、この決定的な上限、validator、文字preflightを代替しない。
...
| `script_manga_runs` | `script_revision_id`と`plan_id`、`predecessor_run_id`、status/phase/approval、件数、設定、評価集計、budget、直近のrun export manifestを保存 |
| `script_manga_run_pages` | runが所有する`page_id`を`page_index`とlayoutへ対応付ける |
| `script_manga_tasks` | コマごとの`panel_spec_json`、ReferenceManifest、round、候補asset、選択asset、score、attempt、`inherited_from_task_id`、`reuse_fingerprint`、将来のrepair/dependency関係を保存 |

`evaluation_json`はtask件数、completed/failed/auditing/awaitingReview、`visualAuditRequired`に加え、successorの継承対象・成功・skip件数とfingerprint versionを記録するrun集計である。候補ごとのVLM結果はtaskの`scores_json.vlmAudit`へ、`state`、model、evaluatedAt、score、passed、4 checks、violationsとして保存する。画像data URL、ファイルパス、プロンプト、生レスポンスは保存しない。`export_manifest_json`は直近のrun exportを保存する。`repair_parent_task_id`と`dependency_task_ids_json`は将来用で、現行経路は実処理に使わない。
...
- `generateImages: false`は`prepared`で止まり、plan、run所有ページ、吹き出し、pending taskをすべて保存する。必須Reference Set不足だけはこの段階で遅延され、ネーム採用後に実際のvisible cast用setを作成できる。
...
- successorの`start`/`resume`はpending投入より先にpredecessorの選択済みassetを照合する。完全一致taskは`completed`へ遷移し、差分taskだけを投入する。
- `retry`は同じtaskをpendingへ戻して新しいroundを作る。既定budgetは1コマ3attemptである。
- `repair`は`awaiting_review`の候補を親に、mask範囲だけをimg2imgする。旧候補を消さず、成功した新assetを同じtaskの候補集合へ追加する。

## API

| method / path | 現行動作 |
| --- | --- |
| `POST /api/projects/:projectId/script-manga-runs` | 通常は最新revisionを固定し、MangaPlanV2、run、所有ページ、taskを作る。`generateImages:false`でprepareのみ。`maxPanelCount`超過は生成前に拒否する。`planningMode:"provided"`、同一`scriptId`の`predecessorRunId`、完全な`successorPlan`を渡すとpredecessorの固定revision/configを引き継ぐsuccessorを作る。`auditMode`は`manual | vlm`、候補policyは`review`のみ |
...
| `POST /api/script-manga-runs/:runId/approve` | validationと必須Reference Setを検査し、承認済みset snapshotを固定して全taskをstrictに再materializeした後に承認。失敗時はrollback |
...
| `POST /api/script-manga-tasks/:taskId/repair` | 保存済み候補の`assetId`、同寸法PNG `maskDataUrl`、任意のdenoise/mask設定を受け、親roundのprompt、seed、workflow revision、sampler、LoRA、参照、poseを固定した局所img2imgを同じtaskへ追加。再現不能な条件はfail-closed |
...
planのPATCHと`successorPlan`は完全な`MangaPlanV2`を要求する。サーバはplan ID、script ID、revision ID、dialogue snapshots、planner provenanceを固定してから検証する。successorでtemplate/provider/LoRA/画像寸法/sampler等を省略した場合はpredecessor configを引き継ぐ。raw APIで`auditMode`を省略した場合は`manual`、Script画面の初期選択は`vlm`である。エージェント向けの作成例と監査手順は[Reference-ScriptMangaAgentWorkflow.md](Reference-ScriptMangaAgentWorkflow.md)を参照する。
...
- successorでは、predecessorで人が選択済みのtaskを一対一に照合する。固定revision、source/dialogue、visible cast、action/props/setting、compiled prompt、Reference Set snapshot、layout/画像寸法、生成設定から成るversioned fingerprintが完全一致した場合だけ、同じassetを新ページへ割り当てて新taskを`completed`にする。asset自体とpredecessorの状態は変更しない。
- reuse fingerprint v4は通常のtxt2img selected assetと、人が採用した局所repair img2img assetを自動継承できる。採用画像bytesのSHA-256と実寸、provider/template、request/intent/native-workflowを署名し、repairでは親asset/round lineage、mask hash、denoise/mask方式と親から固定すべき非repair条件も照合する。旧taskは凍結round/asset情報が完全な場合だけ遅延署名し、不足・画像差替え・lineage不整合時はfail-closedで再生成する。
- fingerprint不一致、未選択候補、既に別の新taskへ対応付けた旧taskは継承しない。`predecessor_run_id`と`inherited_from_task_id`でlineageを追跡できる。
...
errorがあるtaskは`blocked`となり、runのmaterializeは422で失敗する。warningは保存するが実行を止めない。例外としてpre-approvalのprepare/adoptionでは`required-reference-missing`だけを遅延し、run承認時のsnapshot固定後のstrict再materializeで必ず検査する。他のerrorはprepare時から遅延しない。

吹き出しは従来どおり0.0352 page-widthを基準にfitするが、MangaPlanV2経路はfit後に0.016未満(`SCRIPT_MANGA_MIN_FONT_SIZE`)へ縮んだ本文を不合格にする。読めない大きさで通過させず、台詞分割またはページ再計画を要求する。実配置した吹き出し領域は`textSafeZones`へ戻し、再コンパイルしたpromptで文字領域を空けるよう指示する。1コマに複数発話を割り当てると先着の発話が良い位置/サイズを占有し後続が下限割れしやすい落とし穴と、事前予測・自動修復の手順は[`Docs/Reference-DialogueAutoLayout.md`](Reference-DialogueAutoLayout.md)§12を参照。
...
round完了時は候補を自動採用しない。dimensionsの有無と生成要求に対するaspect ratio差（許容8%）を決定的に採点し、`candidate_asset_ids_json`と`scores_json`へ保存する。新規runは`candidateSelectionPolicy: "review"`だけを受け付け、manual/VLMのどちらも`select` APIによる明示採用後だけ`completed`になる。

- `auditMode: "manual"`: メタデータscoreを保存して直ちに`awaiting_review`へ進む。API名は後方互換で、内蔵VLMを使わない外部エージェントまたは人間の明示レビューを表す。
...
- `passed`は推薦情報であり採用権限ではない。不合格候補も削除せず、制作依頼で許可された外部エージェントまたは人間が原寸画像・メタデータ・違反理由を見て採否を決める。
...
局所repairは候補カードの「マスク編集」から既存asset detailを開き、白い修復範囲を確定して「このマスクでコマ修復」を実行する。クライアントは物語promptやworkflowを変更できず、`assetId`、mask、`0 < denoise < 1`、masked content、padding、featherだけを渡す。サーバは親候補の実寸、実seed、template version/hash、生成設定、Reference Setまたは安全に複製したround attachment、pose attachmentを固定する。再現不能な参照、pose、変更済みtemplate、非対応inpaint workflowではtxt2imgへ戻さず失敗する。失敗、0 asset、round欠落でも旧候補があればtaskは`awaiting_review`へ戻り、旧候補を選択できる。
...
Script画面の「MangaPlan V2 / 一括生成」カードからworkflow template、planning mode、`targetPageCount`、`panelsPerPage`、`maxDialoguesPerPanel`、`maxPanelCount`、dialogue policy、audit mode、pose controlを選び、常に`generateImages:false`と人間review policyでprepareする。plan warning、revision、page/panel数、run status/phaseを表示し、承認、生成開始、再開、更新、キャンセルを操作できる。VLM statusは非ブロッキングに取得し、ready、on-demand load可能、unreachable、未設定、manual/OFFを区別する。

生成後はtaskごとにmedium thumbnail、原寸リンク、VLM score/check/violation/modelを並べ、採用ボタンから明示的にselectする。VLMのqueued/deferred/unavailableも区別して表示する。「このコマを再生成」は同じtaskをbudget内でretryし、「マスク編集」は既存asset detailへ入り局所repair候補を追加する。completed runにはPNG/JPEG/PPTX/ORAのdownload buttonを表示する。現行UIでは`preserve | adapt | fill`を選べるが、plan JSONを直接編集するvisual editorはまだない。
...
- VLM violationから修復領域を自動mask化するrepair planner
...
- layout/PanelSpecをGUIで直接編集するplan editor、VLM違反領域のoverlay表示
- `generate` dialogue policyのlexical-similarity gate、一般的な照応解決、衣装・負傷・小物所有を更新する高度なworld-state tracker

task retryはコマ全体を再生成する。局所repairは手動maskから同じtaskの候補へ戻るが、まだVLM violationから自動的には呼ばれない。
...
- `src/server/scriptMangaRepair.test.ts` / `scriptMangaRepair.integration.test.ts`: 入力境界、親条件固定、lineage、旧候補保持、旧/new round選択、template変更時fail-closed
...
- 2026-07-16: ネーム候補を人間が採用した後、そのplanの実visible cast用Reference Setを作る順序へ変更。prepare/adoptionでは必須参照不足だけを遅延し、run承認時にsnapshot固定とstrict再materializeを同一transactionで行う。
- 2026-07-16: `auditMode:"manual"`を内蔵VLMなしの外部エージェント/人間による明示レビューとして整理し、UIとエージェント運用をClaude Code固定ではないプロバイダ非依存表記へ変更。
- 2026-07-15: ネーム密度指定と全入口で再検査する`maxPanelCount`/`maxDialoguesPerPanel` guard、action段落の瞬間境界、N1のページ/台詞密度上限、完全な`successorPlan`によるsuccessor run、reuse fingerprint v4（採用画像hash・旧task遅延署名・repair親round/固定条件対応）、同task候補へ戻る手動mask局所repair API/UIを追加。delivery/cast根拠を構造化し、台詞の脚本順・scene・action由来visible castをfail-closedで検証して、別scene source混入と特定作品由来のprompt形容・常時SF既定を排除。エージェント運用は[`Reference-ScriptMangaAgentWorkflow.md`](Reference-ScriptMangaAgentWorkflow.md)へ分離。
