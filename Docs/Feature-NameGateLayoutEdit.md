...
### 操作性の追補(2026-07-18 第2弾)

- **ドラッグ範囲選択**(コマ枠タブ): パネル/背景からドラッグすると選択矩形が出て、矩形内の頂点を
  全コマ横断で複数選択できる。選択中はいずれかの頂点をドラッグすると全頂点が一括移動
  (T字交差点のような「片側にしか頂点が無い」箇所も、両側の頂点を囲んで選択すれば同時に動かせる)。
  クリック(4px未満の移動)は従来どおりパネル選択。Escで選択解除。
- **ハンドルの形状**: 境界移動=◆(琥珀のひし形)、ガター幅=⇔(青のヒット円+法線方向の両矢印)、
  交差点=●(桃色の丸)で、色だけでなく形から役割が分かる。
- **裁ち切りの限定**: 裁ち切りプレビュー/スナップは**ページの辺と平行な軸沿いの外周辺のみ**
  (法線の主軸成分 ≥0.98、約11.5度以内)。斜め辺は対象外(スナップが軸整合へ潰す事故を防ぐ)。
- **undo/redo**: コマ枠タブ・スタジオ編集の両方で Ctrl+Z / Ctrl+Shift+Z(またはCtrl+Y)+ツールバーの
  ↩/↪ボタン。スナップショット2スタック(snapshotHistory.ts、上限50)。コマ分割の undo はレイアウトのみ
  復元し、移行済みコマ割り当ては戻らない。
