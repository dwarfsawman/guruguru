...
- 2026-07-15: 課題A(cf7d3ad)・課題B(セリフタブ集約+Beatアコーディオン)・課題C(Shift複数選択+グループ、d169c17)を全て main へマージ、実装完了。B-3(手動吹き出しの自動割り付け除外)は現状コードで構造的に保証されていることをコード検証で確認(dialogue_placements 経由でのみ対象選定されるため)。
