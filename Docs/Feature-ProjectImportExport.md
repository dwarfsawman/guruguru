...
指定されたテスト用E02データを隔離DBで測定した。実装3世代の結果を同じ表に示す。

| 項目 | 旧実装 | STORE化・8並列化後 | Rust実装 |
|---|---:|---:|---:|
| エクスポート処理 | 39.91秒 | 4.18秒 | 4.18秒* |
| ディスク書込込み | 40.55秒 | 4.87秒 | 4.87秒* |
| インポート主処理 | 5.75秒 | 2.73秒 | 2.108秒** |
| ファイル読込／受信込み | 6.17秒 | **3.04秒** | 4.045秒 |
| エクスポート終了時RSS | 3,831MiB | 2,706MiB | 2,706MiB* |
| インポート終了時RSS | 1,992MiB | 2,356MiB | 618.8MiB |

\* Rust導入ではインポート経路だけを変更しており、エクスポートはSTORE化後と同じ実装。そのため
エクスポート3項目はSTORE化・8並列化後の実測値を引き継いでいる。

\** JSZip 2世代の「インポート主処理」はファイルをBufferへ読み込んだ後から完了まで、Rustは既に
ディスク上にあるアーカイブの展開開始から完了までの3回中央値。内部境界が異なる参考値である。
利用者が待つ全工程の比較には「ファイル読込／受信込み」を使う。Rustの4.045秒は
チャンク受信→一時ファイル→展開→DB取込の全工程を含む。

全工程ではRust実装はSTORE化・8並列化後の旧JSZip直接Buffer（3.04秒）より約33%遅いが、
終了時RSSを2,356MiBから618.8MiBへ約74%削減した。Rust化の主目的は速度だけでなく、
