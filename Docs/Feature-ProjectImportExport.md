...
- 2026-07-14: 形式名を`.guruzip`へ変更。HTTP受信を外部一時ファイルへストリーミングし、Rust native helperの
  事前安全検査＋8 worker＋1MiB再利用bufferでZIPを直接展開する方式へ移行。1,198,036,728 bytesのE02実データを
  隔離DBで各3回・順序交互に測定。アーカイブが既にディスク上にある条件の中央値は、JSZip
  3.203秒/2,410.1MiBに対しRust 2.108秒/645.4MiB（約1.52倍、ピークRSS約73%減）。本番と同じ
  チャンク受信→一時ファイルを含むRust経路は4.045秒、ピーク655.7MiB、終了時618.8MiB。受信込みA/B
  （両engineを同じ一時ファイル経路に通す診断条件）はJSZip 4.798秒に対しRust 4.045秒。
  `scripts/benchmark-project-import.mjs`の`--repeat`/`--transport`で再計測可能。
