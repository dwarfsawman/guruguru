# 計画: エージェント用 ComfyUI コンテナ + 自動漫画生成実験

作成: 2026-07-11。更新: 2026-07-15。ステータス: **P0〜P2A 完了**。

## 1. 方針

Codex GUI、GURUGURU、Git はホスト上で動かす。エージェント専用 dev コンテナや egress firewall は
採用せず、GURUGURU の永続agent instance分離を使う。コンテナ化するのは ComfyUI のみとし、同じ
イメージをローカル GPU と将来の RunPod で利用する。

守るべき規則:

- GURUGURUの継続的なエージェント作業は`bun run start:agent`（既定5199）、リポジトリ外の
  `GURUGURU_AGENT_DATA_DIR`にある独立永続DBで起動する。自動テスト/使い捨てsmokeだけはtest DBを使う。
...
- ホストの既存 models ルートは `/models` にread-only mountし、agent固有モデルは専用volume
  `/agent-models`へ置く。モデルはイメージに含めない。
...
- `comfyui/Dockerfile`: CUDA 12.8.1、Python 3.11、PyTorch cu128、ComfyUI、
  `PaoloC68/ComfyUI-PuLID-Flux-Chroma`、`kohya-ss/ComfyUI-Anima-LLLite`をcommit固定して導入。
...
ホスト側は`bun run start:agent`を安全ランチャーとする。ランチャーはbuild後、agent modeと外部永続
data dirを強制し、5177を拒否する。
...
| ComfyUI Anima LLLite | `6701c8d6fc3bf1b2ed966b87c95ce609c52cebea` |
...
7. GURUGURUを5199 + agent mode + 外部永続data dirで起動し、設定URLを8288にできる。
8. Anima INT8のtxt2img、LLLite inpaint、pose ControlNetを低負荷条件で完走する。
...
1. `bun run start:agent`でホストGURUGURUを起動し、ComfyUI URLを8288にする。
...
| P2 | ホストagent launcher + health/model check + 手順 | 完了 |
| P2A | Anima INT8 + LLLite inpaint/pose ControlNet API | 完了 |
...
- 当時の`bun run start:test`検証は、2026-07-15に永続`bun run start:agent`へ置換した。
...
## 9. 2026-07-15 Anima INT8検証結果

- `bun run start:agent`を`127.0.0.1:5199`・`instanceMode=agent`・`%LOCALAPPDATA%\\GURUGURU-AGENT`で起動し、health/capabilities APIを確認した。
- ユーザー共有modelsはread-onlyを維持し、agent固有INT8/LLLite weight用の`comfy-agent-models` volumeを追加した。
- Civitai version `3128953`のAesthetic v1.1 INT8と、Anima LLLiteのinpaint-v2/pose-1を配布元SHA-256と照合した。
- RTX 4070 SUPER上で、512×512・8 steps・batch 1のtxt2img / LLLite inpaint / pose ControlNetが各1画像を出力した。
- agent APIの`model-check?family=anima`でbase INT8、encoder、VAE、`animaInpaint`、`animaControlnet`がすべてavailableになった。

## 変更履歴

- 2026-07-15: ホストを永続agent instanceへ分離し、agent専用model volume、Anima INT8 + LLLite inpaint/pose ControlNetのAPI/実機検証を追加。
