...
> この欠陥台帳と根本原因はP1着手前の2026-07-12時点の履歴である。通信/V.O.分類、`adapt`/`fill`、モデル方言などの現行挙動は [Feature-MangaPlanV2.md](Feature-MangaPlanV2.md) と各 `Reference-*.md` を正とする。
...
- **C5 セリフ種別（実装前）**: semanticKindの導出が `(M)`/`(N)`/`SFX:` 規約だけで、通信/V.O./機械音声が通常発話になっていた。現行はterminal cueをexact tokenとして分類し、deliveryと人物の画面内根拠を分離している。
...
- **C7 セリフ生成（実装前）**: `dialoguePolicy` が `preserve` のみだった。現行は原文一致の`adapt`とcaption/monitor/SFXを派生する`fill`まで実装し、自由生成の`generate`だけを拒否する。
...
| speaker名全体が `機械音声/システム/アナウンス` 等、または表示speaker cueと全文《…》readoutの組合せ | dialogue | `machine`(角丸矩形) | なし |
...
| fill monitor(《…》) | narration | `monitor`(UI風角枠) | なし |
   ...
   - 日英のterminal cue、複合cue、部分一致してはいけない人物名、通常人物による《引用》を汎用fixtureにする。特定作品の名前や本文だけをproduction規則へ埋め込まない。
   - `telecom / vo / machine / monitor` は口元アンカーの対象外とし、そのdelivery単独では話者をvisible castへ加えない。ただし同じpanelのaction/sourceが話者の物理的な姿を根拠づける場合は人物を残す。
...
- 2026-07-15: 欠陥台帳を実装前スナップショットと明記し、現行の構造化delivery、action根拠、`adapt`/`fill`の説明へ更新。特定作品fixtureに依存しない回帰方針へ改訂。
- 2026-07-13: 構図の自由化を追加実装([Reference-MangaCompositions.md](Reference-MangaCompositions.md))。裁ち切り/斜めゴマ/figureスロット付き内蔵レイアウト、LLM監督へのlayout guide(説明+figureSlot)、ぶち抜き立ち絵の自動切り抜き(白背景生成→縁フラッドフィル背景除去+白フチ→front帯ImageObject)、吹き出しソルバーの顔回避ゾーン・コマ専有率上限(0.45、strict→relax)を導入。「全コマが均一枠の矩形」「吹き出しが顔やコマ大半を覆う」問題(ALICE E02実機出力)への対処。
- 2026-07-12: P2〜P5を完了。N1ページネーム、キャラシートrun/採用binding、caption/monitor/SFX、集中線/スピード線、画像統計・OCR・参照embedding gate、自動リロール、lettering監査を追加。Docker sandboxでALICE冒頭5ページ10コマを実生成し全候補review待ち、統計/OCR 10/10 passを確認。
- 2026-07-12: P1残件とP2〜P5の基盤を実装。workflow template方言メタデータ、scene bible、balloonStyle全経路、文節折返し/縦中横、adapt原文一致分割、fillのheading/monitor抽出、崩壊統計ゲート/自動リロール、lettering後監査を追加。全822テスト成功。
