...
2. **リーダー** — [Plan-NameReaderUI.md](Plan-NameReaderUI.md) D1をそのまま吸収: SVGワイヤーフレーム背景+コマbboxへの
   ...
   `top=y/page.height` 変換+bleedクランプ。アプリ内全画面では不透明背景を使い、1ページ/右綴じ2ページ、高さ合わせ/横幅合わせを
   切替可能にする。`ArrowLeft` / `ArrowRight`で前後、`Home` / `End`で先頭/末尾へ移動し、見開きは2ページ単位で送る。
   RTL見開きは現在ページを右、次ページを左へ置き、奇数末尾は右側の片ページとして残す。
...
- 2026-07-16: ネーム全画面へ1/2ページ（右綴じ）、高さ/横幅フィット、矢印・Home/End操作を追加。2ページ側のコマ詳細も所属ページから解決する。
