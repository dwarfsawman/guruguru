...
- `Docs/` 直下(`Feature-*.md` / `Plan-*.md`) — 進行中の機能設計・計画ドキュメント
  ...
  - [Feature-MangaPlanV2.md](Feature-MangaPlanV2.md) — immutableなrevision/dialogue/layout/provenance、run lifecycle、P2 UI、LM Studio VLM監査、候補review、successor継承、手動mask局所repair、run exportを担う漫画制作制御層
  ...
  - [Feature-PptxExport.md](Feature-PptxExport.md) — PPTX エクスポート。既存の画像一括書き出しエンドポイントに format="pptx" を追加し、TypeScript生成OOXMLをRustでpackして1ページ=1スライドのデッキを生成
  - [Feature-NameGateLayoutEdit.md](Feature-NameGateLayoutEdit.md) — 人間ゲートのコマ割り修正。スタジオ上の辺/頂点/交差点/ガター/裁ち切り/吹き出し位置のドラッグ編集、candidate の customLayouts/balloonHints レイヤー、set-custom-layout API、layoutSnapshot への固定と preferredCenter ソルバー連携
...
- `Docs/ReferenceFlows/` — 参照用 ComfyUI ワークフロー JSON(API フォーマット)
  ...
  - [Reference-AnimaUnifiedSwitchWorkflow.json](ReferenceFlows/Reference-AnimaUnifiedSwitchWorkflow.json) — Anima の txt2img / img2img / inpaint / LoRA 統合ワークフロー。動的Anima LLLite ControlNetを含む解説は [Reference-AnimaPreset.md](Reference-AnimaPreset.md)
...
| [Reference-AnimaPreset.md](Reference-AnimaPreset.md) | Anima のモデル構成、統合 Switch、Versioned Reference Set、face + full_body LatentBatch、family別fallback境界 |
| [Reference-AgentInstanceApi.md](Reference-AgentInstanceApi.md) | 5199の永続エージェント向けインスタンス、Anima INT8のinpaint/pose ControlNet API契約と起動境界 |
| [Reference-MangaCompositions.md](Reference-MangaCompositions.md) | 漫画の自由な構図: 裁ち切り/枠なし/斜めゴマのプリセット、コマぶち抜き立ち絵(figure スロット→自動切り抜き ImageObject)、吹き出しの顔回避・専有率上限。LLM 監督/provided plan からの使い方 |
| [Reference-ScriptMangaAgentWorkflow.md](Reference-ScriptMangaAgentWorkflow.md) | エージェントによるFountain漫画制作の現行手順。ネーム密度、prepare監査、visible cast、successor asset継承、pose/inpaint境界、JPG/PPTX書き出し |
| [Reference-FileExports.md](Reference-FileExports.md) | PNG/JPEG ZIP・PPTX・ORAのRust pack、ファイル成果物ライフサイクル、圧縮方式、計測ログ |
...
| [Feature-ReferenceSets.md](Done/Feature-ReferenceSets.md) | Versioned face/full-body Reference Set、人間承認UI、Chroma/Anima経路、Manga Run固定snapshotとpreflight |
...
- 2026-07-15: `Reference-AgentInstanceApi.md`を追加。ユーザー5177と分離した永続agent instance、Anima INT8 + LLLite inpaint/pose ControlNetのAPI契約を整理。
- 2026-07-15: `Reference-ScriptMangaAgentWorkflow.md`を追加。prepare-firstのネーム監査、台詞要素上限（既定4）/コマ数guard、画面外発話、画像hashとrepair lineageを含むsuccessor継承、pose/inpaintとfontの現行境界を整理。
- 2026-07-15: `Reference-FileExports.md`を追加。画像ZIP/PPTX/ORAのRust pack・ファイルストリーミングと、WebSAM alpha/coalesce最適化の現行仕様を反映。
- 2026-07-13: `Reference-MangaCompositions.md` を新設。裁ち切り/斜めゴマ/ぶち抜き立ち絵プリセット、figure スロットの自動切り抜き(白背景生成→背景除去+白フチ→ImageObject)、吹き出しソルバーの顔回避・コマ専有率上限を実装し、LLM 監督へレイアウトガイド(figureSlot 付き)を渡すようにした。
- 2026-07-13: Versioned Reference Setを追加。face/full-body承認UI、Chroma PuLID / Anima LatentBatch、Manga Run固定snapshot、全promptへのappearance注入を実装。隔離8288ではidentity改善12/12、768最大7.33 GiB、1024選抜最大7.48 GiB・OOM 0を確認し、製品既定を768に決定。
