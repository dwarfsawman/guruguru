...
指定candidate/run/plan/taskの所属を照合してから正規`guiUrl`を返す。エージェントはそのURLを開かず、出力をそのまま人間へ渡す。
...
2. agent:cli contextで実インスタンスとID対応を検証し、返されたguiUrlを直ちに人間へ案内
   (エージェント自身はURLを開かない。以後エージェントはネームに触らない。人間が開いたページは5秒毎に
   ライブ更新され、追加候補・レイアウトフリップ・採用が自動で映る)
3. agent:cli wait candidatesで同エンドポイントを15〜30秒毎にポーリング:
...
5. agent:cli wait runで GET /api/script-manga-runs/:runId をポーリング:
