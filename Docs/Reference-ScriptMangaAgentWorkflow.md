...
OpenAI互換接続のLocal LLMへ1回の`count:3`で再生成する。外部サブエージェントは可読性・映画的・テンポ重視の
独立案を別途比較する用途には有効だが、現行の候補作成APIへprovided planを直接importしてA/B/Cタブへ載せることはできない。
外部案を使う場合は、比較後に選んだ1案を`planningMode:"provided"`のrunとしてvalidation/preflightへ通す。
