...
test("in-flight+保留 debounce の状態で flush すると、in-flight を待たず即時発射し両方の完了で resolve する", async () => {
  const { timers, fake, persister } = setup();
  persister.schedule();
  timers.fire();
  await drainMicrotasks();
  ...
  // クローズ時の不変条件: flush は in-flight 完了を待たずにその場で発射する
  // (persist が state クリア前に同期でボディを確定できるようにするため)。
  assert.equal(fake.calls.length, 2, "flush はその同期実行内で2発目を出す");
  assert.equal(fake.calls[0]!.context.isStale(), true, "並走する旧発射の応答は stale");
  assert.equal(fake.calls[1]!.context.isStale(), false);

  fake.calls[1]!.resolve();
  await drainMicrotasks();
  assert.equal(flushed, false, "旧 in-flight の完了も待ってから resolve する");
  fake.calls[0]!.resolve();
  await flushPromise;
  assert.equal(flushed, true);
});

test("persist は launch を決めた同期実行内で呼ばれる(クローズ時の state クリアより先にボディ確定できる)", async () => {
  const timers = createFakeTimers();
  // lightbox セッション state の代役: persist は実行時にこれを読む(実装の規約と同じ)。
  let sessionState: { pageId: string } | null = { pageId: "page-1" };
  const capturedPageIds: Array<string | null> = [];
  const persister = createDebouncedPersister({
    persist: () => {
      capturedPageIds.push(sessionState ? sessionState.pageId : null);
  ...
  persister.schedule();
  // closePagePanelLightbox と同じ順序: flush() を呼んだ直後(同期)に state をクリアする。
  const flushPromise = persister.flush();
  sessionState = null;
  await flushPromise;
  assert.deepEqual(capturedPageIds, ["page-1"], "persist は state クリア前に同期で呼ばれている");
});

test("flush がコアレス待ちを先取りした場合、待ち側 Promise は新発射の完了へ相乗りし二重発射しない", async () => {
  const { timers, fake, persister } = setup();
  persister.schedule();
  timers.fire();
  await drainMicrotasks();
  ...
  const queuedPromise = persister.persistNow(); // in-flight 中の要求 → コアレス待ち
  let flushed = false;
  const flushPromise = persister.flush().then(() => {
    flushed = true;
  });
  assert.equal(fake.calls.length, 2, "flush が待ちを先取りして即時発射する");
  ...
  assert.equal(flushed, true);
  await drainMicrotasks();
  assert.equal(fake.calls.length, 2, "先取り後にコアレス待ち分を二重発射しない");
});
