/**
 * ページオブジェクトの「ギズモ外接矩形」を作る(Docs/Feature-CGCollectionSuite.md P1/P2)。
 * box は `size` をそのまま使う。text はレイアウトの bbox(`textLayoutClient.ts` のクライアント側 LRU
 ...
 */
import { PAGE_OBJECT_MIN_SIZE, type BoxObject, type TextObject } from "../shared/pageObjects";
...
export function gizmoBoxForPageObject(object: BoxObject | TextObject): GizmoBox {
  if (object.kind === "box") {
