/**
 * 完成品の画像一括書き出し(Docs/Feature-CGCollectionSuite.md P4)ダイアログ。Book のページ一覧
 * (見出し「画像書き出し」/選択モードの「選択ページを画像書き出し」)から開く。既存の
 * `.workflow-modal`/`.workflow-dialog`(レイアウトテンプレピッカーの前例)を踏襲し、値は
...
export function renderImageExportModal(
  ...
  return `
    <div class="workflow-modal image-export-modal" role="presentation">
      <section class="workflow-dialog image-export-dialog" role="dialog" aria-modal="true" aria-label="画像書き出し">
        <header class="workflow-dialog-header">
          <div>
            <p class="section-kicker">Book · 画像書き出し</p>
            <h2>画像を書き出す</h2>
          </div>
        ...
        <footer class="image-export-footer">
          ...
          <button class="button-primary image-export-submit${busy ? " is-busy" : ""}" type="button" data-action="submit-image-export" ${busy ? "disabled" : ""}>
            ${busy ? `<span class="button-spinner" aria-hidden="true"></span>書き出し中…` : `${iconImage()}書き出す`}
          </button>
        </footer>
      </section>
    </div>
