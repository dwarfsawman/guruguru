...
import { pptxTypefaceForFontId } from "./pptxExport.ts";
...
test("pptx text uses the selected font without changing the general default", () => {
  const fonts = [
    { id: "noto", familyName: "Noto Sans JP", subfamilyName: "Bold", source: "system" as const },
    { id: "genei", familyName: "源暎アンチック v5", subfamilyName: "Regular", source: "system" as const }
  ];
  assert.equal(pptxTypefaceForFontId("default", fonts), "Noto Sans JP");
  assert.equal(pptxTypefaceForFontId("noto", fonts), "Noto Sans JP");
  assert.equal(pptxTypefaceForFontId("genei", fonts), "源暎アンチック v5");
});
...
test("createImageExport(format=pptx): PowerPoint標準の吹き出し図形と編集可能文字を出力する", async () => {
  ...
  assert.match(slideXml, /<a:ea typeface="[^"]+"\/>/);
});

// Docs/Feature-ScreenTones.md: トーンは balloon/box/text と違い PowerPoint ネイティブ図形化できない
// (seed 付き PRNG で作るドット/線パターンを OOXML 図形として表現する手段が無い)。renderSlidePage の
// フィルタから漏れると背景 PNG にも slide 図形にも現れず静かに消えるため、そのリグレッションを防ぐ。
test("createImageExport(format=pptx): トーンは編集可能図形化されず、背景 PNG へ平坦化される", async () => {
  const { projectId, pageIds } = await setupProject(1);
  const toneObject = {
    id: "tone_1",
    kind: "tone",
    ...
    clipPanelId: null,
    seed: 1,
    params: defaultToneParams("halftone")
  };
  runSql("UPDATE pages SET objects_json = ? WHERE id = ?", [JSON.stringify([toneObject]), pageIds[0]!]);
  const result = await createImageExport(projectId, { pageIds, format: "pptx" });
  const zip = await JSZip.loadAsync(result.buffer);
  const slideXml = await zip.file("ppt/slides/slide1.xml")!.async("string");

  // 編集可能図形としては出さない(balloon/box/text 用の <p:sp>/<p:grpSp> がページ画像以外に無い)。
  assert.doesNotMatch(slideXml, /tone_1/);
  assert.doesNotMatch(slideXml, /<p:sp>/);
  assert.doesNotMatch(slideXml, /<p:grpSp>/);

  // 代わりに背景 PNG(image1.png)へラスタライズされ、トーン無しページと比べてピクセルが変化している。
  const withToneImage = Buffer.from(await zip.file("ppt/media/image1.png")!.async("nodebuffer"));
  const baseline = await setupProject(1);
  const baselineResult = await createImageExport(baseline.projectId, { pageIds: baseline.pageIds, format: "pptx" });
  const baselineZip = await JSZip.loadAsync(baselineResult.buffer);
  const baselineImage = Buffer.from(await baselineZip.file("ppt/media/image1.png")!.async("nodebuffer"));
  assert.notEqual(Buffer.compare(withToneImage, baselineImage), 0, "tone should be rasterized into the background image");
});
