...
test("guruzip ラウンドトリップ: 進行中ステータスは failed へ正規化される(§4)", async () => {
  ...
  const task = getRow<{ status: string; inherited_from_task_id: string | null; reuse_fingerprint: string | null; reuse_source_json: string | null }>(
    "SELECT status, inherited_from_task_id, reuse_fingerprint, reuse_source_json FROM script_manga_tasks WHERE run_id IN (SELECT id FROM script_manga_runs WHERE project_id = ?)",
    [newProjectId]
  );
  assert.equal(task!.status, "failed");
  assert.equal(task!.inherited_from_task_id, null);
  assert.equal(task!.reuse_fingerprint, null);
  assert.equal(task!.reuse_source_json, null);
});

test("guruzip ラウンドトリップ: completed task の再利用署名はID・パス再配置後の遅延再署名へ戻す", async () => {
  const fixture = await buildFixtureProject("completed-reuse-resign");
  runSql("UPDATE generation_rounds SET status = 'completed' WHERE id = ?", [fixture.roundId]);
  runSql("UPDATE generation_jobs SET status = 'completed' WHERE id = ?", [fixture.jobId]);
  runSql("UPDATE script_manga_runs SET status = 'completed', phase = 'completed' WHERE id = ?", [fixture.runId]);
  runSql(
    `UPDATE script_manga_tasks
     SET status = 'completed', asset_id = ?, selected_asset_id = ?, candidate_asset_ids_json = ?,
         reuse_fingerprint = 'pre-import-fingerprint', reuse_source_json = ?
     WHERE id = ?`,
    [
      fixture.assetId,
      fixture.assetId,
      JSON.stringify([fixture.assetId]),
      JSON.stringify({ version: "script-manga-selected-asset-reuse/v4", roundId: fixture.roundId }),
      fixture.taskId
    ]
  );

  const exported = await exportProject(fixture.projectId);
  const imported = await importProject(exported.buffer);
  const task = getRow<{
    status: string;
    ...
    `SELECT status, selected_asset_id, reuse_fingerprint, reuse_source_json
     FROM script_manga_tasks
     WHERE run_id IN (SELECT id FROM script_manga_runs WHERE project_id = ?)`,
    [String(imported.project.id)]
  );

  assert.equal(task?.status, "completed");
  assert.ok(task?.selected_asset_id, "the reviewed selection itself remains imported");
  assert.equal(task?.reuse_fingerprint, null);
  assert.equal(task?.reuse_source_json, null);
});

test("guruzip ラウンドトリップ: exporting の run は failed へ正規化される", async () => {
  const fixture = await buildFixtureProject("exporting-status");
  runSql("UPDATE script_manga_runs SET status = 'exporting', phase = 'exporting' WHERE id = ?", [fixture.runId]);

  const exportResult = await exportProject(fixture.projectId);
  const importResult = await importProject(exportResult.buffer);
  const importedRun = getRow<{ status: string; last_error_json: string | null }>(
    "SELECT status, last_error_json FROM script_manga_runs WHERE project_id = ?",
    [String(importResult.project.id)]
  );

  assert.equal(importedRun!.status, "failed");
  assert.match(importedRun!.last_error_json ?? "", /imported while in progress/);
});
