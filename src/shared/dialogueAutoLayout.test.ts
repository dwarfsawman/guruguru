...
test("runDialogueAutoLayout: preferredCenter のヒント位置近傍に配置される", () => {
  const layout = twoPanelLayout();
  // ヒント無しでは上部優先で置かれるが、コマ下部を指定すると下部へ寄る。
  const preferred: PageVec = { x: 0.75, y: 1.2 };
  const hinted = runDialogueAutoLayout({
    layout,
    existingObjects: [],
    items: [item({ placementId: "p1", lineId: "l1", orderIndex: 0, preferredCenter: preferred, size: { x: 0.14, y: 0.12 } })],
    seed: 7
  });
  assert.equal(hinted.unplacedPlacementIds.length, 0);
  const placed = hinted.objects[0]!;
  assert.ok(Math.hypot(placed.position.x - preferred.x, placed.position.y - preferred.y) < 0.05,
    `placed at ${placed.position.x},${placed.position.y}`);

  const unhinted = runDialogueAutoLayout({
    layout,
    existingObjects: [],
    items: [item({ placementId: "p1", lineId: "l1", orderIndex: 0, size: { x: 0.14, y: 0.12 } })],
    seed: 7
  });
  assert.ok(unhinted.objects[0]!.position.y < 0.5, "ヒント無しは従来どおり上部優先");
});

test("runDialogueAutoLayout: preferredCenter が障害物と衝突する場合は従来探索へフォールバックする", () => {
  const layout = twoPanelLayout();
  const preferred: PageVec = { x: 0.75, y: 0.7 };
  const obstacle: PageObject = {
    id: "existing",
    kind: "box",
    position: preferred,
    rotation: 0,
