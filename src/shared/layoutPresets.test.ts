...
test("selectScriptMangaLayoutId: splash→裁ち切り、large位置×強調スロット一致、全mediumは候補先頭", () => {
  assert.equal(selectScriptMangaLayoutId(["splash"]), "builtin:splash-bleed");
  assert.equal(selectScriptMangaLayoutId(["medium"]), "builtin:splash");
  assert.equal(selectScriptMangaLayoutId(["medium", "medium", "medium"]), "builtin:three-horizontal");
  assert.equal(selectScriptMangaLayoutId(["large", "medium"]), "builtin:two-bleed-hero-top");
  assert.equal(selectScriptMangaLayoutId(["large", "medium", "medium"]), "builtin:three-hero-top");
  assert.equal(selectScriptMangaLayoutId(["medium", "large", "medium"]), "builtin:three-side-hero");
  assert.equal(selectScriptMangaLayoutId(["medium", "medium", "large"]), "builtin:three-hero-bottom",
    "figure レイアウトも hero@2 に整合するが、事前選択では figure を避ける");
  assert.equal(selectScriptMangaLayoutId(["medium", "large", "medium", "medium"]), "builtin:four-vertical-hero");
  assert.equal(selectScriptMangaLayoutId(["medium", "medium", "medium", "large"]), "builtin:four-hero-bottom");
  assert.equal(selectScriptMangaLayoutId(["large", "medium", "medium", "medium", "medium"]), "builtin:five-hero-top");
  assert.equal(selectScriptMangaLayoutId(["medium", "medium", "medium", "medium", "large"]), "builtin:five-panel");
  assert.equal(selectScriptMangaLayoutId(["medium", "medium", "large", "medium", "medium", "medium"]), "builtin:six-hero-right");
  // 整合可能な候補が無い hero 位置は、目標面積比への L1 フィットで穏当な候補へ倒す。
  assert.equal(selectScriptMangaLayoutId(["medium", "large"]), "builtin:two-horizontal");
  assert.equal(selectScriptMangaLayoutId(["large", "medium", "medium", "medium"]), "builtin:four-grid");
