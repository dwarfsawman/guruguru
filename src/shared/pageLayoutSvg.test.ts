...
test("renderPageWireframeSvg: visualScale塗り分け・台詞量バー・ビートkind・turnHookマーク・diff枠", () => {
  const layout = findLayoutPreset("builtin:three-hero-top")!.layout;
  const svg = renderPageWireframeSvg(layout, {
    panels: [
      { visualScale: "large", dialogueCharacters: 60, beatKinds: ["reveal"] },
      { visualScale: "medium", beatKinds: ["reaction"] },
      { visualScale: "small" }
    ],
    turnHook: "reveal",
    highlight: true,
  ...
  assert.ok(svg.includes("--wire-hero"), "large塗り");
  assert.ok(svg.includes("--wire-small"), "small減光");
