...
test("effectiveGradientPoints: 未指定は angle 由来の領域両端、指定時はそのまま、退化(2点一致)は angle へフォールバック", () => {
  ...
  const fallback = effectiveGradientPoints({ angle: 0 }, hx, hy);
  ...
  const explicit = effectiveGradientPoints({ angle: 0, gradStart: { x: -0.02, y: 0.03 }, gradEnd: { x: 0.05, y: -0.01 } }, hx, hy);
  assert.deepEqual(explicit.start, { x: -0.02, y: 0.03 });
  assert.deepEqual(explicit.end, { x: 0.05, y: -0.01 });

  const degenerate = effectiveGradientPoints({ angle: 0, gradStart: { x: 0.01, y: 0.01 }, gradEnd: { x: 0.01, y: 0.01 } }, hx, hy);
...
test("renderToneSvg: gradient は gradStart/gradEnd 未指定なら「angle 由来の実効2点を明示指定」と同一出力(後方互換)", () => {
  ...
  const points = effectiveGradientPoints(baseParams, size.x / 2, size.y / 2);
